...
//       loadWorkOrders();
//     }
//   }, [user, currentPage, pageSize, activeFilters, technicianFilter, sortOrder]); // Added sortOrder
...
const WorkOrderTable = ({ onBack }) => {
  ...
  }, [user, currentPage, pageSize, activeFilters, technicianFilter, sortOrder]);
  ...
  }, [technicianFilter, user, currentPage, pageSize, sortOrder]);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-[1600px] mx-auto p-6">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {kpiRecords.map((record) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-sm text-slate-600">
                              {record.responseTimeMinutes || "-"}
                            </td>
                            <td className="px-4 py-3 text-sm text-slate-600">
                              {record.resolutionTimeMinutes || "-"}
                            </td>
                            <td className="px-4 py-3 text-sm text-slate-600">
                              {record.actualResponseTimeMinutes || "-"}
                            </td>
                            <td className="px-4 py-3 text-sm text-slate-600">
                              {record.actualResolutionTimeMinutes || "-"}
                            </td>
                            <td className="px-4 py-3 text-sm text-slate-600">
                              {record.totalHoldTimeMinutes || "-"}
                            </td>
                            <td
                              className="px-4 py-3 text-sm text-slate-600 max-w-xs truncate"
                              title={record.responseTimeNote}
                            >
                              {record.responseTimeNote || "-"}
                            </td>
                            <td
                              className="px-4 py-3 text-sm text-slate-600 max-w-xs truncate"
                              title={record.resolutionTimeNote}
                            >
                              {record.resolutionTimeNote || "-"}
                            </td>
                            <td
                              className="px-4 py-3 text-sm text-slate-600 max-w-xs truncate"
                              title={record.holdNote}
                            >
                              {record.holdNote || "-"}
                            </td>
                          </tr>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
              <div className="flex justify-between items-center p-6 border-b border-slate-200">
                <h3 className="text-lg font-semibold text-slate-900 flex items-center">
                  ...
                  Cancel Work Order WO-{selectedWorkOrder.workOrderId}
                </h3>
                <button
                  onClick={() => {
                    setShowCancelModal(false);
              ...
              <div className="p-6">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-slate-500 resize-none"
                  rows={4}
                />
              </div>
              <div className="p-6 border-t border-slate-200 flex justify-end gap-3">
        ...
        {/* Note Modal */}
        {showNoteModal && selectedWorkOrder && selectedStatus && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
              <div className="flex justify-between items-center p-6 border-b border-slate-200">
                <h3 className="text-lg font-semibold text-slate-900 flex items-center">
              ...
              <div className="p-6">
                <p className="text-sm text-slate-600 mb-4">
                  A note is required for changing the status to{" "}
                  <strong>
                    {
                ...
                {selectedStatus === "START" && kpiRecord && (
                  <div className="mb-4">
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                      Response Time Note
                    </label>
              ...
              <div className="p-6 border-t border-slate-200 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-slate-800 rounded-lg hover:bg-slate-900 disabled:opacity-50 transition-colors"
                >
                  {updatingStatus ? (
                    <>
                      <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
