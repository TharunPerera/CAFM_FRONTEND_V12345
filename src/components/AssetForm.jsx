...
const AssetForm = ({ isEdit = false }) => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        if (isEdit && assetId) {
          ...
          // Load contracts for the asset's company
          const contractsResponse =
            await companyService.getContractsByCompanyId(assetData.companyId);
          ...
          await Promise.all([
            ...
            loadBuildings(assetData.contractId, assetData.subZoneId),
            loadVillaApartments(assetData.contractId, assetData.buildingId),
            loadFloors(assetData.contractId, assetData.villaApartmentId),
            loadRooms(assetData.contractId, assetData.floorId),
          ]);
  ...
  const loadContracts = async (companyId) => {
    ...
    try {
      const response = await companyService.getContractsByCompanyId(companyId);
  ...
  const handleChange = (e) => {
    ...
    if (name === "companyId") {
      ...
      loadContracts(value);
      // Reset contractId and dependent fields
      setAsset((prev) => ({
        ...prev,
        contractId: "",
        zoneId: "",
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    disabled={!asset.companyId}
                    className={`w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
                      errors.contractId
                        ? "border-red-500 bg-red-50"
                        : "border-gray-200 hover:border-gray-300"
                    } ${
                      !asset.companyId ? "bg-gray-100 cursor-not-allowed" : ""
                    }`}
                  >
