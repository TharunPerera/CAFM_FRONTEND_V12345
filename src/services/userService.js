...
// //   // Legacy methods for backward compatibility
// //   getAllUsers: () => api.get("/users/internal?page=0&size=1000"),
// //   getUserById: (id, userType) => api.get(`/users/${id}/${userType}`),
...
//     api.get(`/users/supervisors?page=${page}&size=${size}`),
//   // Filtered users
//   getFilteredUsers: (filterData, page = 0, size = 10, sort = "desc") =>
//     api.post("/users/filter", filterData, { params: { page, size, sort } }),
//   // Legacy methods for backward compatibility
//   getAllUsers: () => api.get("/users/internal?page=0&size=1000"),
//   getUserById: (id, userType) => api.get(`/users/${id}/${userType}`),
//   createUser: (userData) =>
...
export const userService = {
  ...
  getFilteredUsers: (filterData, page = 0, size = 10, sort = "desc") => {
    const updatedFilterData = {
      ...filterData,
      userType: filterData.userType || "INTERNAL", // Default to INTERNAL
    };
    return api.post("/users/filter", updatedFilterData, {
      params: { page, size, sort, sortField: "firstName" },
    });
  },
  // Legacy methods for backward compatibility
  getAllUsers: () => api.get("/users/internal?page=0&size=1000"),
